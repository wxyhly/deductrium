...
class NoFreeSystem {
    ...
    getReplVarsBoolOrItem(ast, res = {}, isItem) {
        if (ast.type === "replvar") {
            res[ast.name] ??= isItem;
        ...
        if (ast.type === "sym") {
            if (quantSyms.includes(ast.name)) {
                this.getReplVarsBoolOrItem(ast.nodes[0], res, true);
                this.getReplVarsBoolOrItem(ast.nodes[1], res, false);
            }
            else {
                const isItem = !logicSyms.includes(ast.name);
                for (const n of ast.nodes) {
                    this.getReplVarsBoolOrItem(ast, res, isItem);
                }
            }
        }
        if (ast.type === "fn") {
            if (ast.name.match(/^#v*nf/) || ast.name.match(/^#c?rp/)) {
                ...
                let first = true;
                for (const n of ast.nodes) {
                    this.getReplVarsBoolOrItem(ast, res, first ? isItem : true);
                    first = false;
                }
            }
        }
    ...
    expand(ast, varLists) {
        // first, check all sub nodes are consist
        if (ast.nodes?.length)
            for (const n of ast.nodes) {
                if (!this.expand(n, varLists))
                    return false;
            }
        // ast is atomvar
        if (ast.type === "replvar")
        ...
        if (nfParams) {
            ...
            if (quantParam) {
                ...
                if (!this.expand(subsub, varLists))
                    return false;
            }
            ...
            if (subnfParams) {
                ...
                if (this.eqQ(sq, quants)) {
                    ...
                    return this.expand(ast, varLists);
                }
                // todo: whether others can be determined?
                this.addNf(ast, quants, vars);
                return true;
...
//# sourceMappingURL=nofree.js.map