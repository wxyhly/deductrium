...
:.i,4$[[hyp]]解锁假设[n]注意：定理表中若有假设[n]不可开启对应门#p
...
:%,0@公理a6解读：[n]#nf($0,$1)的值为$0[n]外加验证变量$1不在[n]表达式$0中自由出现[n](nf为nofree缩写)
...
:%*,5@[n]注意：变量都是“项”[n]不能说某项为真或假[n]只有公式才能讨论其真假[n]纯命题逻辑都是公式[n]即都可判断真假
...
:%*,4@a8公理使用提示2[n]若想把$a=$b中的$a替换成$c[n]则没问题，因为$a第一个出现[n]是一定要被替换的。因此[n]让系统只替换第一个变量[n]不再管后面的变量即可
:%*,2$获取30µg推理素
:%,1@这里离解锁皮亚诺公理[n]ZFC公理与类型层[n]仅有三个方块距离了
:%,1#[n]$0=$1,$1=$2⊢$0=$2[n]#d
:%,2#通过此门需消耗推理素100µg
:%*,5$[[type]]解锁类型层
...
:%,1#通过此门需消耗推理素100µg
:%,2;1;1;1@
:%,1@其实若解锁了[n]充要替换元定理[n]就可直接生成：[n]先证( ⊢ (~(($0>$1) > ~($1>$0)) <> ($0<>$1)))[n]然后使用元定理[n]把aExt的内容作为$$2[n]$$3填0（意思为全部替换）
:%,1#通过此门需消耗推理素66µg
:%,1@
...
:%,0#[n]a@{b} <> a=b[n]#p
:%*,0#通过此门需消耗推理素5mg
:%*,0,5$[[simplezfc]]解锁ZFC简化大礼包[n]即用更方便的[n]人类习惯的符号[n]重新书写ZFC所有公理
:%,5#[n]{a}={b} <> a=b[n]#p
:%*,5#[n]~(a@{{a}})[n]#p
:%*,5,2$获取4.2mg推理素
:%,0#[n]{a,b}={b,c} <> a=c[n]#p
...
:%*,3$[[ttua]]解锁同伦类型论[n]泛等公理[n]ua[n](等价即相等)[n]#t
