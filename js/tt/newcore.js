    ...
    check(ast, context, ignoreErr) {
        ...
        if (ast.type === "apply") {
            ...
            if (tfn.nodes && this.equal(tfn.nodes[0], tap, context)) {
                ...
                const _rootStr = this.state.activeAst ? parser.stringify(this.state.activeAst) : "";
                const _pos = _rootStr.indexOf(_fnStr);
                const _posStr = _pos >= 0 ? String(_pos + 1) : "?";
                const _expStr = tfn.nodes ? parser.stringify(tfn.nodes[0]) : "?";
...
//# sourceMappingURL=newcore.js.map