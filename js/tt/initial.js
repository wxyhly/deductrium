...
export function initTypeSystem() {
    ...
    typeName = "fnext";
    addRule("@构造", "@fnext:Pa:U_.Pb:U_.Pf:a->b.Pg:a->b.(Px:a.eq (f x) (g x))->(eq f g)");
    addRule("@构造", "fnext:=@fnext _ _ ");
    addRule("_构造", "fnext");
    addRule("定义", "@happly:=(λa:U_.(λb:U_.(λf:(a→b).(λg:(a→b).(λh:eq f g.(λx:a.trans (λg:(a→b).eq (f x) (g x)) h rfl))))))");
    addRule("定义", "happly:=@happly _ _");
    addRule("@计算", "@fnext_happly:Pa:U_.Pb:U_.Pf:a->b.Pg:a->b.Px:eq f g.eq (fnext f g (happly f g x)) x");
    addRule("@计算", "@happly_fnext:Pa:U_.Pb:U_.Pf:a->b.Pg:a->b.Px:Px:a.eq (f x) (g x).eq (fnext f g (happly f g x)) x");
    ...
    addRule("定义", "@eqvrefl:=(λu:U@.(λa:(Uu).(pair (Lf:a->a.(Sg:a->a,Px:a,eq x (g(f x)))X(Sh:a->a,Px:a,eq x (f(h x)))) (λx:a.x) ((pair (λg:(a→a).(Πx:a,eq x (g x))) (λx:a.x) (λx:a.rfl)),(pair (λh:(a→a).(Πx:a,eq x (h x))) (λx:a.x) (λx:a.rfl)))))):(Πu:U@,(Πa:(Uu),((eqv a a))))");
    addRule("定义", "eqvrefl:=@eqvrefl _:Pa:U_.eqv a a");
    addRule("定义", "@id2eqv:=(λu:U@.(λa:(Uu).(λb:(Uu).(λh:eq a b.trans (λx:(Uu).eqv a x) h (@eqvrefl u a))))):(Πu:U@,(Πa:(Uu),(Πb:(Uu),((eq a b)→(eqv a b)))))");
    ...
    addRule("@构造", "@ua:Pu:U@,Pa:Uu,Pb:Uu,(eqv a b)->(eq a b)");
    addRule("@构造", "ua:=@ua _ _ _");
    addRule("_构造", "ua");
    addRule("@计算", "@ua_id2eqv: Pu:U@.Pa:Uu.Pb:Uu.Pm:eq a b.eq (ua (id2eqv m)) m");
    addRule("@计算", "@id2eqv_ua: Pu:U@.Pa:Uu.Pb:Uu.Pm:eqv a b.eq (id2eqv (ua m)) m");
    ...
    addRule("_计算", "apd_loop : eq (apd ?C (ind_S1 ?C ?cb ?cl) loop)  ?cl");
    typeName = "I";
    addRule("类型", "I:U");
    addRule("构造", "0I:I");
    addRule("构造", "1I:I");
    ...
    addRule("_计算", "apd_segI : eq (apd ?C (ind_I ?C ?c0 ?c1 ?cs) segI)  ?cs");
    typeName = "Sus";
