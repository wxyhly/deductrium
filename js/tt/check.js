...
export class HoTT {
    ...
    expandDefinition(ast, context) {
        ...
        while (ast.type === "apply") {
            ...
            if (!this.equal(t2, t1.nodes[0], context))
                throw "函数与作用目标类型不匹配:函数" + this.print(ast.nodes[0]) + "需要接受的参数类型为" + this.print(t1.nodes[0]) + "，作用对象" + this.print(ast.nodes[1]) + "类型为" + this.print(t2);
    ...
    check(ast, context = {}) {
        ...
        if (ast.type === "apply") {
            ...
            const t2 = this.check(ast.nodes[1], context);
            if (!this.equal(t2, t1.nodes[0], context))
                throw "函数与作用目标类型不匹配:函数" + parser.stringify(ast.nodes[0]) + "需要接受的参数类型为" + parser.stringify(t1.nodes[0]) + "，作用对象" + parser.stringify(ast.nodes[1]) + "类型为" + parser.stringify(t2);
