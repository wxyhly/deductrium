...
let listInfos = [];
let consts = new Set;
function addTerm(list, previnfo) {
    ...
    ast2HTML(idx, ast, scopes = [], context = {}, userLineNumber = 0) {
        ...
        if (ast.type === "var") {
            ...
            if (astname.match(/^[1-9][0-9]*$/)) {
                ...
                el.classList.add("constant");
                if (astname.startsWith("ind_"))
                    el.classList.add("ind_fn");
                if (constructors.has(astname))
                    el.classList.add("constructors");
                if (macro.has(astname))
            ...
            switch (ast.type) {
                ...
                case "S":
                    ...
                    const newType = this.hott.clone(ast.nodes[0]);
                    this.hott.unbeautify(newType);
        ...
        for (const node of spans) {
            ...
            node.addEventListener('mouseover', ev => {
                ...
                try {
                    astType = new Core().checkType(astType, localCtxt);
                ...
                try {
                    floatTypeDiv.appendChild(this.ast2HTML("", astType, scopes, localCtxt, userLineNumber));
                }
                catch (e) {
    ...
    queryType(typeStr) {
        ...
        for (const e of this.getInhabitatArray()) {
            ...
            try {
                ...
                if (ast.type === ":") {
                    if (this.hott.equal(ast.nodes[1], ref, {}))
                        return true;
                }
                else {
                    if (this.hott.equal(this.hott.check(ast), ref, {}))
                        return true;
                }
            }
            catch (e) {
    ...
    unlock(str) {
        switch (str) {
            ...
            case "indFalse":
                ...
                break;
            case "simplFn":
                this.hott.features.add(HoTTFeatures.SimplFnType);
                break;
        }
        this.updateTypeList();
