    ...
    execMetaDeduct() {
        ...
        switch (cmdBuffer[1]) {
            case "q":
                ...
                try {
                    formalSystem.deductions[formalSystem.metaQuantifyAxiomSchema(
                        Number(cmdBuffer[2]), this.astparser.parse(cmdBuffer[3])
                    )].from = `*mq d${cmdBuffer[2]}`;
