...
export function initTypeSystem() {
    ...
    addRule("@计算", "@ind_True _ _ ?ctrue true === ?ctrue");
    ...
    addRule("@计算", "@ind_Bool _ _ ?c0b _ 0b === ?c0b");
    addRule("@计算", "@ind_Bool _ _ _ ?c1b 1b === ?c1b");
    ...
    addRule("@计算", "@ind_nat _ _ ?c0 _ 0 === ?c0");
    addRule("@计算", "@ind_nat ?u ?C ?c0 ?csucc (succ ?n) === ?csucc ?n (@ind_nat ?u ?C ?c0 ?csucc ?n)");
    typeName = "(nat)";
    ...
    addRule("@计算", "ind_eq2 ?C ?crfl ?x ?x (@refl ?1 ?2 ?x) === ?crfl ?x");
    addRule("@计算", "ind_eq2 ?C ?crfl ?x ?x rfl === ?crfl ?x");
    ...
    addRule("_定义", "apd");
    addRule("计算", "apd ?C ?f (refl ?x) === refl (?f ?x)");
    addRule("@计算", "apd ?C ?f rfl === rfl");
    ...
    addRule("@计算", "transconst rfl _ = rfl");
    ...
    addRule("@构造", "@pair : Pu:U@,Pv:U@,Pa:Uu,Pb:a->Uv,Pxa:a,Pxb:b xa,@Prod u v a (Lx:a,b x)");
    ...
    addRule("@计算", "@ind_Sum _ _ _ _ _ _ ?cinl _ (@inl _ _ _ _ ?xa) === ?cinl ?xa");
    addRule("计算", "ind_Sum ?C ?cinl ?cinr (inr ?xb) === ?cinr ?xb");
    addRule("@计算", "@ind_Sum _ _ _ _ _ _ _ ?cinr (@inr _ _ _ _ ?xb) === ?cinr ?xb");
    ...
    addRule("@计算", "@ap_loop : Pu:U@.Pa:Uu.Pcb:a.Pcl:cb=cb.(ap (rec_S1 a cb cl) loop) = cl");
    addRule("@计算", "ap_loop := @ap_loop _");
    addRule("_计算", "ap_loop");
    ...
    addRule("@计算", "@apd_segI : Pu:U@,PC:I->Uu,Pc0:(C 0I),Pc1:(C 1I),Pcs: @eq u (C 1I) (trans C segI c0) c1,eq (apd C (@ind_I u C c0 c1 cs) segI) cs");
    addRule("@计算", "apd_segI := @apd_segI _ _ _ _ _");
    addRule("_计算", "apd_segI : (apd ?C (ind_I ?C ?c0 ?c1 ?cs) segI) = ?cs");

    typeName = "Sus";
    addRule("@类型", "@Sus:Pu:U@. Uu -> Uu");
    addRule("@类型", "Sus:=@Sus _");
    addRule("_类型", "Sus");
    addRule("@构造", "@North:Pa:U_.Sus a");
    addRule("@构造", "North:=@North _");
    addRule("_构造", "North");
    addRule("@构造", "@South:Pa:U_.Sus a");
