...
type AST = {
    type: string, name?: string, nodes?: AST[], checked?: AST, err?: any
}
type Context = {
    [varname: string]: AST,
}
...
export class Core {
    ...
    checkConst(n: string) {
        ...
        if (res) return res;
        let ulevel = 0;
        if (ulevel = UniverseLevel.is(n)) return UniverseLevel.gen(ulevel + 1);
    ...
    expandDef(ast: AST, context: Context, count = 0): boolean {
        return false;
    }
}
class NatLiteral {
...
class UniverseLevel {
    ...
    static is(name: string): number {
        if (!name || name[0] !== "U") return 0;
        if (name === "U") return 1;
    ...
    static gen(n: number): AST {
        if (n === 1) return wrapVar("U");
        if (n > 1) return wrapVar("U" + (n - 1));
    }
    static merge(lv1: number, lv2: number) {
