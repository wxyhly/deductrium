...
type State = {
    ...
    /** internal use for replacing infered vals */
    activeAst?: AST;
...
export class Core {
    ...
    replaceVarInChecked(ast: AST, name: string, dst: AST, context?: Context) {
        if (ast.checked) this.replaceVar(ast.checked, name, dst, context);
    ...
    replaceVar(ast: AST, name: string, dst: AST, context?: Context, fvDst = Core.getFreeVars(dst)) {
        if (ast.type === "var") {
            ...
            if (context) {
                for (const e of fvDst) {
                    if (context[e] && Core.exactEqual(context[e], ast.nodes[0])) { bounded = true; break; }
                }
            }
    ...
    checkConst(n: string) {
        ...
        if (["add", "natO", "leqO"].includes(n) && res?.checked) return res.checked;
    ...
    equal(a: AST, b: AST, context: Context, moveSemantic?: boolean, depth = Infinity) {
        ...
        if ((a.type === "->" && b.type === "P") || (b.type === "->" && a.type === "P")) {
            if (
                this.equal(a.nodes[0], b.nodes[0], context, moveSemantic, depth) &&
        ...
        if ((a.type === "," && b.type === "apply") || (b.type === "," && a.type === "apply")) {
            ...
            let xb = b.type === "," ? a : b;
            if (
                this.equal(xa.nodes[1], xb.nodes[1], context, moveSemantic, depth) &&
        ...
        if (a.type === b.type && (a.type === "L" || a.type === "P" || a.type === "S")) {
            if (!this.equal(a.nodes[0], b.nodes[0], context, moveSemantic, depth)) return false;
        ...
        if (context["*"]) {
            UniverseLevel.reduceLvl(a); UniverseLevel.reduceLvl(b);
        ...
        if (a1.type === b1.type && a1.name == b1.name && a1.nodes?.length === b1.nodes?.length) {
            ...
            for (let i = 0; i < a1.nodes?.length; i++) {
                if (!this.equal(a1.nodes[i], b1.nodes[i], context, true, depth)) { breaked = true; break; }
            }
        ...
        if (modified) return this.equal(a1, b1, context, true, depth - 1);
        return false;
    }

    mergeInfered(added: Context, context: Context, depth = Infinity) {
    ...
    reduce(ast: AST, dependent?: boolean): boolean {
    ...
    expandDef(ast: AST, specified?: Set<string>): boolean {
        if (ast.type === "var") {
            if (specified && !specified.has(ast.name)) return false;
        ...
        if (ast.nodes.length) {
            for (const n of ast.nodes) {
                if (this.expandDef(n, specified)) modified = true;
            }
        }
        return modified;
    }
        ...
        for (const matched of applyRes) {
            ...
            if (fn === "addO" && matched.length > 3) {
                ...
                if (matched[2].name === "0O") {
                    Core.assign(ast, matched[3], true); modified = true; continue;
                }
                // add succ x y : succ(add x y)
                if (matched[2].type === "apply" && matched[2].nodes[0].name === "succO") {
                    Core.assign(ast, wrapApply(wrapVar("succO"), wrapApply(wrapVar("addO"), matched[2].nodes[1], matched[3])), true); modified = true; continue;
                }
            ...
            if (fn === "leqO" && matched.length > 3) {
                // 0 <= a : True
                if (matched[2].name === "0O") {
                    Core.assign(ast, wrapVar("True"), true); modified = true; continue;
                }
                if (matched[2].type === "apply" && matched[2].nodes[0].name === "succO") {
                    // succ a <= 0 : False
                    if (matched[3].name === "0O") {
                        Core.assign(ast, wrapVar("False"), true); modified = true; continue;
                    }
                    // succ a <= succ b : a <= b
                ...
                if (matched[2].type === "apply" && matched[2].nodes[0].name === "limO") {
                    Core.assign(ast, {
                        type: "P", name: "n", nodes: [wrapVar("nat"), wrapApply(
                            wrapVar("leqO"), wrapApply(matched[2].nodes[1], wrapVar("n")), matched[3]
                        )]
                    }, true);
                    modified = true; continue;
                }
