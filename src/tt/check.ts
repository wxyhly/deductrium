...
export class HoTT {
    features = new Set<HoTTFeatures>;
    axioms = axioms;
    definitions = definitions;
    beautifys = beautifys;
    beginTime: number;
    beginTimeFlag: number;
    startTimer() {
    ...
    replaceVar(ast: AST, name: string, dst: AST, fvDst = this.getFreeVars(dst)) {
        if (ast.type === "var") {
            ...
            if (!fvDst.has(ast.name)) this.replaceVar(ast.nodes[1], name, dst, fvDst);
            else {
    ...
    checkProof(witness: AST, theorem: AST, context: Context = {}) {
    ...
    equal(ast1: AST, ast2: AST, context: Context) {
        ...
        if (ast1.nodes?.length) {
            for (let i = 0; i < ast1.nodes.length; i++) {
                if (!this.equal(ast1.nodes[i], ast2.nodes[i], i ? ctxt : context)) return false;
            }
        }
        return true;
    }
    expandDefinition(ast: AST, context: Context): boolean {
        ...
        if (!ast.nodes?.length) return modified;
        const ctxt = Object.assign({}, context);
        if (ast.type === "L" || ast.type === "P" || ast.type === "S") {
            ...
            for (const n of ast.nodes) modified ||= this.expandDefinition(n, ctxt);
        }
    ...
    match(ast: AST, pattern: AST, res: { [variable: string]: AST } = {}) {
        ...
        if (ast.name && ast.name !== pattern.name) {
            if (pattern.name.padEnd(ast.name.length, "'") !== ast.name) return null;
        ...
        if (ast.nodes?.length) {
            for (let i = 0; i < ast.nodes.length; i++) {
                if (!this.match(ast.nodes[i], pattern.nodes[i], res)) return null;
            }
        }
        return res;
    }
    ...
    check(ast: AST, context: Context = {}): AST {
        ...
        if (ast.type === "var") {
            ...
            if (varType) return this.clone(varType);
    ...
    expandHomotopy(ast: AST, context: Context) {
        const typeA = this.check(ast.nodes[0], context);
        const typeB = this.check(ast.nodes[1], context);
        if (!this.equal(typeA, typeB, context)) throw "同伦关系符“~”两边类型不相等";
        if (typeA.type !== "P") throw "同伦关系符“~”仅能对函数作用";
    ...
    expandEquivalence(ast: AST, context: Context) {
    ...
    expandComposite(ast: AST, context: Context) {
        const [f, g] = ast.nodes;
    ...
    beautify(ast: AST, shrinkDef: boolean = true) {
        let modified = false;
        if (shrinkDef) {
        ...
        if (!ast.nodes?.length) return modified;
        for (const n of ast.nodes) if (this.beautify(n, shrinkDef)) modified = true;
    ...
    unbeautify(ast: AST) {
        ...
        if (!ast.nodes?.length) return modified;
        for (const n of ast.nodes) if (this.unbeautify(n)) modified = true;
        this.normalizeNatLiteral(ast);
        if (modified) {
            this.unbeautify(ast);
        }
    ...
    normalizeNatLiteral(ast: AST) {
        if (ast.type === "apply" && ast.nodes[0].name === "succ") {
            ...
            for (const n of ast.nodes) this.normalizeNatLiteral(n);
        }
        return ast;
    }
    expandIndNatLiteral(ast: AST): boolean {
        let modified = false;
        if (ast.type === "apply" && this.isNatLiteral(ast.nodes[1])) {
            if (ast.nodes[0].type === "apply" && ast.nodes[0].nodes[0].type === "apply" && ast.nodes[0].nodes[0].nodes[0].type === "apply") {
                ...
                for (const n of ast.nodes) modified ||= this.expandIndNatLiteral(n);
            }
            return modified;
        }
    }
    addNatLiteral(n1: string, n2: string) {
    ...
    expandOperatorNatLiteral(ast: AST): boolean {
        ...
        if (ast.type === "apply" && this.isNatLiteral(ast.nodes[1])) {
            ...
            for (const n of ast.nodes) modified ||= this.expandOperatorNatLiteral(n);
        }
        return modified;
    }
