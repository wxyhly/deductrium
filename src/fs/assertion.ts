...
export class AssertionSystem {
    ...
    getReplVarsType(ast: AST, res: ReplvarTypeTable = {}, isItem: boolean): ReplvarTypeTable {
        if (ast.type === "replvar") {
            res[ast.name] ??= isItem;
            if (res[ast.name] !== isItem) throw `变量${ast.name}不能同时为项和公式`;
            return res;
        }
    ...
    match(ast: AST, pattern: AST, replNameReg: RegExp, isItem: boolean, result: ReplvarMatchTable, varTable: ReplvarTypeTable, assertions: AST[]) {
        if (pattern.type === "replvar" && pattern.name.match(replNameReg)) {
            ...
            if (!astmgr.equal(result[pattern.name], ast)) throw `模式匹配失败：匹配多个替代变量${pattern.name}时值不相同`;
            return;
        }
        ...
        if (ast.type !== pattern.type || ast.name !== pattern.name) throw "模式匹配失败";
        if (ast.nodes?.length !== pattern.nodes?.length) throw "模式匹配失败";
        if (ast.nodes?.length) {
    ...
    checkGrammer(ast: AST, type: "m" | "d" | "p" | "v" | "i", consts: Set<string>) {
        ...
        if (type === "m") throw "未找到元推理符号"; if (type === "d") throw "未找到推理符号";
        if (ast.type === "sym") {
            if (quantSyms.includes(ast.name)) {
                if (type !== "p") return "意外出现了量词" + ast.name;
                const varName = this.getVarName(ast.nodes[0]);
                if (!varName) throw `非变量表达式出现在了量词${ast.name}的约束变量中`;
            ...
            if (verbSyms.includes(ast.name)) {
                if (type !== "p") throw "意外出现谓词符号" + ast.name;
                this.checkGrammer(ast.nodes[0], "i", consts);
                this.checkGrammer(ast.nodes[1], "i", consts);
                return;
            }
            if (logicSyms.includes(ast.name)) {
                if (type !== "p") throw "意外出现逻辑连词" + ast.name;
        ...
        if (ast.type === "fn") {
            if (ast.name === "#rp" || ast.name === "#crp") {
                if (ast.nodes?.length !== 3 && ast.nodes?.length !== 4) throw `系统函数${ast.name}的参数个数必须为三个或四个`;
            ...
            if (ast.name.match(/^#v*nf$/)) {
                if (!(ast.nodes?.length > ast.name.length - 2)) throw `系统函数${ast.name}的参数个数必须至少有${ast.name.length - 1}个`;
                this.checkGrammer(ast.nodes[0], type, consts);
    ...
    assertUnwrap(ast: AST, varTypes: ReplvarTypeTable) {
        ...
        if (nfp) {
            for (const v of nfp[2]) {
                if (this.nf(v, nfp[0], nfp[1]) !== T) throw `变量${v}自由出现断言失败`;
            }
            return;
        }
        const crp = this.getCrpParams(ast);
        if (crp) {
            if (this.crp(...crp, varTypes) !== T) throw `表达式可被替换断言失败`;
            return;
        }
        throw "未找到断言函数";
    }
