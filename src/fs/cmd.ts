    ...
    execCmdBuffer() {
        ...
        try {
            ...
            if (this.gui.unlockedMacro) switch (cmdBuffer[0]) {
                ...
                case "hyp": if (this.gui.unlockedHyp) return this.execHyp();
            }
        ...
        try {
            ...
            switch (cmdBuffer[1]) {
                ...
                case "c":
                    ...
                    break;
                case "f":
                    newName = formalSystem.metaNewFunction([cmdBuffer[2], cmdBuffer[3], cmdBuffer[4], cmdBuffer[5]].map(
                        v => this.astparser.parse(v)
                    ), "元规则生成*");
                    break;
                case "cdt":
        ...
        if (curLength < condLength + 2) {
            ...
            try {
                formalSystem.deduct({
                    ...
                    replaceValues: cmdBuffer.slice(2 + condLength).map(
